        ...
        <div class="flex flex-col md:flex-row gap-4 mt-4">
            <div class="md:w-3/4">
                <div class="bg-white overflow-x-auto rounded-lg shadow-md p-6 mb-4">
                    <table class="w-full min-w-full table-auto border-collapse border-spacing-0">
                        ...
                        <tbody>
                            @foreach ($order_items as $item)
                                <tr wire:key="{{ $item->id }}" class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                                    <td class="py-4 px-4">
                                        <div class="flex items-center space-x-4">
                                            <!-- Gambar -->
                                            <img class="h-12 w-12 rounded-md object-cover hidden lg:block" src="{{ url('storage', $item->product->images[0]) }}" alt="Image {{ $item->product->name }}">
                                            <!-- Nama Produk -->
                                            <span class="font-semibold text-gray-800 dark:text-gray-200 break-words">
                                                {{ $item->product->name }}
                                            </span>
                                        </div>
                                    </td>
                                    <td class="py-4 px-4 text-gray-700 dark:text-gray-300 whitespace-nowrap">
                                        Rp. {{ number_format($item->unit_amount, 0, ',', '.') }}
                                    </td>
                                    <td class="py-4 px-4 text-center text-gray-800 dark:text-gray-200">
                                        {{ $item->quantity }}
                                    </td>
                                    <td class="py-4 px-4 text-gray-700 dark:text-gray-300 whitespace-nowrap">
                ...
                <div class="bg-white overflow-x-auto rounded-lg shadow-md p-6 mb-4">
                    <h1 class="text-3xl font-bold text-slate-500 mb-3">Shipping Address</h1>
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                            <p class="text-gray-800 dark:text-gray-300">{{ $order->address->address . ', ' . $order->address->city . ', ' . $order->address->state . ', ' . $order->address->zip_code }}</p>
                        </div>
                        <div>
                            <p class="font-semibold text-gray-800 dark:text-gray-300">Phone:</p>
                            <p class="text-gray-700 dark:text-gray-400">{{ $order->address->phone }}</p>
                        </div>
                        <div>
                            <p class="font-semibold text-gray-800 dark:text-gray-300">Email:</p>
                            <p class="text-gray-700 dark:text-gray-400">{{ $order->address->email }}</p>
                        </div>
                    </div>
                </div>
